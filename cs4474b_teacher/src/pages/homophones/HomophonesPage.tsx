...
export function HomophonesPage({
  ...
  const fetchHomophones = async (word: string, setId: string) => {
    ...
    try {
      const response = await fetch(
        `https://api.datamuse.com/words?sl=${word}&max=4`,
      );
      ...
      pushUpdate(
        sets.map((s) => (s.id === setId ? { ...s, words: finalWords } : s)),
      );
    } catch (e) {
  ...
  const getDuplicateError = (
    currentSet: HomophoneGame,
    allSets: HomophoneGame[],
  ) => {
    ...
    const isDuplicateSet = allSets.some((s) => {
      ...
      return (
        s.id !== currentSet.id &&
        sActive.length > 0 &&
        [...sActive].sort().join(",") === currentSorted
      );
    });
  ...
  const clearAllSets = () => {
    if (
      window.confirm(
        "Are you sure you want to clear all sets? This cannot be undone.",
      )
    ) {
      pushUpdate([]);
    }
    ...
    <div className="HomophonesPage">
      ...
      <main className="HomophonesPage__list">
        {sets.map((set, index) => {
          ...
          const activeSentences = (set.exampleSentences || []).filter(
            (s) => s?.trim() !== "",
          );
          ...
          const isIncomplete =
            !hasEnoughWords || !hasSentence || duplicateError !== null;

          // FEEDBACK STUFF: red for empty, yellow for partially filled, hidden when done
          const isEmpty =
            activeWords.length === 0 && activeSentences.length === 0;
          ...
          return (
            ...
            >
              ...
              <div className="HomophoneCard__header">
                <h3 className="HomophoneCard__title">
                  ...
                  return (
                    <div key={i} className="WordInputWrapper">
                      <TextInput
                        ...
                        onChange={(e) =>
                          handleWordChange(set.id, i, e.target.value)
                        }
                        onKeyDown={(e) =>
                          e.key === "Enter" &&
                          i === 0 &&
                      ...
                      {word && (
                        <button
                          className="WordInputWrapper__clear"
                          onClick={() => deleteWord(set.id, i)}
                        >
                          ×
                        </button>
                      )}
              ...
              <div className="HomophoneCard__sentences">
                <label className="HomophoneCard__label">
                  Example Sentences
                </label>
                {[0, 1].map((i) => (
                  <textarea
                    ...
                    onChange={(e) =>
                      handleSentenceChange(set.id, i, e.target.value)
                    }
                  />
                ))}
              </div>
              ...
              {isIncomplete && (
                <div className="HomophoneCard__errors">
                  {!hasEnoughWords && (
                    <p className="HomophoneCard__error-text">
                      • Needs 2+ words
                    </p>
                  )}
                  {!hasSentence && (
                    <p className="HomophoneCard__error-text">
                      • Needs 1+ example sentences
                    </p>
                  )}
                  {duplicateError && (
                    <p className="HomophoneCard__error-text">
